...
可作为全局时间序列对称模式挖掘算法的对称度阈值。为与第4章
提出的分段对称度阈值进行区分，全局对称度阈值用$\theta_1$来表示。
...
在未指定先验阈值的情况下，如果根据算法~\ref{alg:threshold1}
...
综上所述，全局时间序列对称模式挖掘算法的计算流程如图
~\ref{fig:algorithm_process}所示。首先对时间序列数据进行标准化，
将其统一到同一个量纲范围内，然后使用全局对称性度量算法计算时间序列的
对称度，之后由用户决定使用先验阈值或者算法~\ref{alg:threshold1}
计算得到对称度阈值，最后通过比较时间序列的对称度是否在阈值范围内
确定其是否属于全局对称模式。
\begin{figure}
  \centering
  \includegraphics[width=0.50\linewidth]{global_symmetry_algorithm.PNG}
  \caption{全局时间序列对称模式挖掘流程图}
...
动态数据集合。一般并不会随着数据点到来度量全部流式时间序列的
对称性，而只会度量指定长度$n$的流式时间序列对称性。
为方便计算，可以把长度为$n$流式时间序列视为全局时间序列。
检测流式时间序列的对称性需要满足实时性和高效性，
即给定一个实时生成的时间序列$X=(p_1,p_2,\dots)$和长度$n$，
需要在每个数据点$p_t$生成时，检测并度量流式时间子序列
$S=\left(p_{t-n+1},p_{t-n+2},\dots,p_t \right)$的对称性。
...
$S=\left(p_{t-n+1},p_{t-n+2},…,p_{t-1} \right)$
范围之内的对称度状态不须重新推导。因为根据3.1节介绍的时间序列
对称模式挖掘算法的数学性质，该算法满足单调性和连续性，时间序列
$S$从$p_{t-n+1}$到$p_{t-1}$之间的对称度状态$DP\left(i,j\right)$
...
整体向右下方移动，并未导致$p_{t-n+1}$到$p_{t-1}$之间产生
...
因此，可以使用循环二维队列$dp$作为保存对称度矩阵的数据结构，
当需要计算新数据点$p_t$的相关状态时，只需要循环的利用
已经失效的循环二维队列第$t \% w+1$列所占空间，进而优化空间复杂度。
此外，由于流式数据是动态、源源不断到来的，模式识别需要实时报警。
因此，可以采用全局对称度阈值确定算法对流式全局对称模式进行分类。
总之，对称模式流式挖掘算法只需要长度$n$和
新增数据点$p_t$作为输入，同时保存
流式全局时间序列$X$，流式对称度矩阵$dp$和
...
展示了时间序列对称模式流式挖掘算法
的计算流程，第1-7行对流式全局时间序列$X$和对称度阈值$d$进行更新，
第8-15行进行冷启动处理，当数据点数小于$n$时更新对称度状态$dp$
并统一判断为非对称模式。第16-20行倒序计算流式时间序列的对称性，
并保存到循环二维队列$dp$中。
第21行将时间序列$X$中第一个数据点删除，以保证下次处理时$X$也
只有$n$个数据点，第22-26行
对数据点数大于$n$的时间序列计算对称度状态，
并通过比较是否超过对称度阈值，判断是否为对称模式。
尽管该算法的空间复杂度为$O(n^2)$，但其时间复杂度却优化至了
$O(n)$，远远高于基于DTW距离的对称模式挖掘算法，
具有较高的时间效率和较快的响应速度。
...
\begin{algorithm}
  ...
  \small
  \begin{algorithmic}
    \REQUIRE 新数据点$p_t$，长度$n$
    \ENSURE 布尔型变量，标志$S=\left(p_{t-n+1},p_{t-n+2},…,p_t \right)$是否为流式全局对称模式

    \STATE 将$p_t$置于流式全局时间序列$X$中
    \IF{$t > 1$}
      \STATE $\theta \leftarrow \theta+D\left(p_{t}, p_{t-1}\right)$
    \ENDIF
    \IF{$t > w$}
      \STATE $\theta \leftarrow \theta-D\left(p_{t-n}, p_{t-n-1}\right)$
    \ENDIF
    ...
    \STATE $i \leftarrow w-1$
    \WHILE{$i > 0$}
      \STATE $dp_{i,t \% w+1} \leftarrow D\left(p_{i}, p_{t} \right) + \min \left(dp_{i,(t-1) \% w+1},dp_{i+1,t \% w+1},dp_{i+1,(t-1) \% w+1}\right)$
      \STATE $i \leftarrow i-1$
    \ENDWHILE

    \STATE 将$p_{t-w+1}$从流式全局时间序列$X$中删除;
    \IF{$\frac{dp_{1,t \% w+1}}{w} > \frac{\theta}{w-1}$}
      \RETURN false
    \ELSE
      \RETURN true
    \ENDIF
...
数据特征，根据数据点差分距离的平均值确定了对称度阈值。
并在3.3节利用对称度阈值过滤得到了全局对称模式。
最后，本文通过优化对称度计算方式
将全局对称模式挖掘算法扩展到了流式数据领域。
根据第5章的实验证明，本节所定义的全局对称模式挖掘算法
在不同来源的数据集上具有更高的准确性和鲁棒性。
而且，由于根据流式时间序列的数据特点对对称度计算方式进行优化，
全局流式对称模式挖掘算法在时间效率上比基于动态时间规整的算法
高出了一个阶数，在实际工业场景具有广泛的应用价值。